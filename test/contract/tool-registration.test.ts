...
//     direct return.
//   - The exact total tool count matches the documented surface (39 tools).
...
describe("tools/list", () => {
  it("returns exactly 39 tools (the documented surface)", async () => {
    const r = await client.listTools();
    expect(r.tools).toHaveLength(39);
  });
  ...
  it("includes the documented set of tool names", async () => {
    ...
    // name is registered. The "exactly 39 tools" count check above catches
...
describe("tools/call — error envelope round-trip via the full SDK transport", () => {
  ...
  //
  // Using a representative subset (one per category) rather than all 39 to
