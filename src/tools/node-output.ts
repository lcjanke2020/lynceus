...
// Pull-based stdout/stderr reader for cdp-mcp-owned Node children.
...
//
// Buffer is populated only when cdp-mcp launched the Node child itself
...
export function registerNodeOutputTools(server: McpServer) {
  registerJsonTool(
    server,
    "get_node_output",
    "Query the buffered stdout/stderr from a cdp-mcp-launched Node process (launch_node). Paginate with `since` (the seq from the last call). Populated only for launch_node sessions; attach_node returns an empty list because cdp-mcp doesn't own the child's stdio. Separate from get_console_logs — that's the V8 inspector's Runtime.consoleAPICalled stream; this is the OS-level pipe.",
    {
