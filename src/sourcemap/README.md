...
| `loader.ts` | `attachScriptListener()`, `decodeDataUri()` | Subscribes to `Debugger.scriptParsed`, populates the store, and fetches the source map. Browser-first via `Network.loadNetworkResource` (so we inherit page auth / cookies / dev-server middleware); Node `fetch` fallback for plain localhost. Handles RFC 2397 multi-parameter `data:` URIs (webpack inline-source-map). |
...
- **Source-map fetch path matters.** `loader.ts` prefers `Network.loadNetworkResource` so we go through the page's origin/CORS/auth context. The Node `fetch` fallback only works for plain localhost. For session-protected source maps the browser path is the one that actually has the cookies.
