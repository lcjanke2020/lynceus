...
| [src/tools/README.md](./src/tools/README.md) | `registerJsonTool` pattern, structured error envelope, full 56-tool catalog with browser/Node/shared kind column. |
...
| `src/framework/` | Stateless `FrameworkAdapter` resolution seam plus the React adapter, exact-pinned backend source loader, and per-session attach/detach bridge lifecycle. Mutable bridge/runtime state remains on the addressed `SessionState`. | [React DevTools design](./docs/react-devtools-design.md) |
...
| `src/tools/` | 56 MCP tools across 14 files: `session` (incl. `launch_chrome` / `attach_chrome` / `launch_node` / `attach_node`), `nav`, `source`, `breakpoints`, `execution`, `inspect`, `console`, `network`, `dom` (incl. structured `locate` / `wait_for` / `get_form_state` LocatorSpec tools), `forms`, `storage`, `node-output` (Node-only `get_node_output`), `timeline` (cross-session `get_timeline`), and `react` (`attach_react_devtools` / `detach_react_devtools`). Plus `_register.ts`, `_session_input.ts`, and `_locator_runtime.ts` (helpers). | [README](./src/tools/README.md) |
