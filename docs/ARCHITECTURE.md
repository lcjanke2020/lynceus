...
- A **tool layer** (`src/tools/`) — 59 thin handlers wrapping CDP calls with Zod schemas + a structured error envelope.
...
flowchart LR
    ...
    Tools["src/tools/<br/>59 tool handlers"]
...
- Browser-only domains (`Page`, `DOM`, `Input`, `Network`) and the main-frame React bridge are not available in Node sessions. The 30 browser-only MCP tools return the structured error envelope `{ error: "unsupported_target", message: "Tool <name> requires a browser session (current session is node)" }` when called against a Node session. The reverse holds for `get_node_output`, which targets a Node-only surface and returns `{ error: "unsupported_target", message: "Tool get_node_output requires a node session (current session is browser)" }` when called against a browser session. The lookup table lives in `src/session/capabilities.ts` (see [`src/tools/README.md`](../src/tools/README.md) §Capability gating).
...
| [`src/framework/`](../src/framework/) | `adapter.ts`, `react.ts`, `react-backend.ts`, `react-store.ts`, `react-inspection.ts`, `react-source.ts` | Framework resolution, exact-pinned React backend loading, attach/detach orchestration, generation-scoped v7 operations materialization, inspection correlation/dehydration, and TypeScript source attribution | [React DevTools design](./react-devtools-design.md) |
...
| [`src/tools/`](../src/tools/) | 14 tool files + `_register.ts` + `_session_input.ts` + `_locator_runtime.ts` | 59 MCP tool implementations across `session` / `nav` / `source` / `breakpoints` / `execution` / `inspect` / `console` / `network` / `dom` / `forms` / `storage` / `node-output` / `timeline` / `react` | [README](../src/tools/README.md) |
